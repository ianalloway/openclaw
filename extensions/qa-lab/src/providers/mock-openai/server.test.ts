...
describe("qa mock openai server", () => {
  ...
  it("detects a WhatsApp location body after channel and timestamp prefixes", async () => {
    const server = await startMockServer();
    const setupInput = makeUserInput(
      "When a later WhatsApp location message shows 37.774900, -122.419400, " +
        "reply with only this WhatsApp location marker: QA_WHATSAPP_LOCATION_OK. " +
    ...
    const timestampResponse = await postResponses(server, {
      stream: false,
      input: [setupInput, makeUserInput("[Tue 2026-07-14 12:47 UTC] 📍 37.774900, -122.419400")],
    });

    expect(response.status).toBe(200);
    expect(outputText(await response.json())).toBe("QA_WHATSAPP_LOCATION_OK");
    expect(timestampResponse.status).toBe(200);
    expect(outputText(await timestampResponse.json())).toBe("QA_WHATSAPP_LOCATION_OK");
  });
  ...
  it("does not treat structured WhatsApp tokens in ordinary prose as message bodies", async () => {
    ...
    const response = await postResponses(server, {
      ...
      input: [
        setupInput,
        makeUserInput(
          "Please compare [Tue 2026-07-14 12:47 UTC] 📍 37.774900, -122.419400 and explain [that] <contact> text",
        ),
      ],
    });
