...
export async function maybeSpawnVisibleSession(params: {
  ...
  try {
    ...
    if (!runId) {
      try {
        const abortResponse = await gatewayCall<{ abortedRunId?: string | null }>(
          "sessions.abort",
          {
            key: childSessionKey,
            agentId: targetAgentId,
        ...
        if (abortResponse.abortedRunId) {
          await deleteVisibleSession(gatewayCall, childSessionKey);
        }
      } catch {
        // Unknown run id: keep session unless abort confirms a stopped run.
      }
      return { status: "error", error: runError, childSessionKey };
    }
    try {
